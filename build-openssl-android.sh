...
if [ -z "$FIPS_SIG" ] || [ ! -e "$FIPS_SIG" ]; then
  ...
  echo "FIPS_1_$_FIPS_SIG"
  if [ ! -e "$_FIPS_SIG" ]; then
    _FIPS_SIG=`find $PWD -name incore`
  fi
  echo "FIPS_2_$_FIPS_SIG"
...
if [ "$_ANDROID_ARCH" == "arch-arm64" ]; then
    ...
    #编译FIPS
    #/usr/local/ssl/$ANDROID_API
    #  -I/usr/local/ssl/$ANDROID_API/include
    ./config fips shared no-comp no-hw no-engine --openssldir=/usr/local/ssl/$ANDROID_API --with-fipsdir=/usr/local/ssl/$ANDROID_API --with-fipslibdir=/usr/local/ssl/$ANDROID_API/lib/
fi
